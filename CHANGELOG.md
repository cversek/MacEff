...
- **amail host tier, the declarations and identity by claim** (`deploy_config.py`, `broker.py`, `audit.py`, `daemons/broker.py`): the addressing file takes `tier: container | host` (default container, so every older file keeps its meaning), `supervision` (required under the host tier, `none` refused) and per-agent `shared_uid`; a uid may repeat only under the host tier and only among agents that all declare it shared, and a host-tier file refuses to load inside a container (`CONTAINER_MARKER`, on the addressing model so both daemons get it from the one file they both read). The broker's `identify` returns an `Identity` -- a str that remembers `how` it was established -- and `settle` resolves a shared uid from the client's claim: bound name accepted, foreign name refused and recorded, no claim refused; on an unshared uid a disagreeing claim is refused exactly as before. Every audit line carries `tier`, and `authorship` reads `claimed:<name>` for a claim and `so_peercred:<name>` for the kernel, never one for the other; the sent-side ledger record carries the same word. The daemon banner names the tier, the supervision and the shared uids. Ten tests (`test_amail_host_tier.py`)

- **amail host tier, the policy half** (`amail.md` 1.4.0, `docs/AMAIL_DEPLOYMENT.md`): §7.5 defines a second, supervision-justified tier for agents on the operator's own machine — identity on a declared shared uid taken from a client claim the audit writes as `claimed:<name>` beside the kernel's `so_peercred:<name>`, the agent-launched broker as the tier's normal case with tier and supervision on the banner and every audit line, refusals where the declaration can be wrong (host file in a container, shared uid without the tier, autostart without the tier, supervision `none`), and an explicit list of what the tier does not change; §1.3 carries the `tier`, `shared_uid` and `supervision` declarations; §3.3 adds the tier and the two authorship words to the audit record; §8 moves host agents from deferred to resolved. The deployment doc gains the declaration, the refusal table and the break table. Code follows in the next changes
