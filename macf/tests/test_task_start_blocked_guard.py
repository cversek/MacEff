...
def wired(monkeypatch):
    ...
    def _run(tasks, task_id, force=False):
        ...
        return cli.cmd_task_start(args), writes, events
...
class TestBlockedStartIsRefused:
    def test_pending_blocker_refuses_and_writes_nothing(self, wired, capsys):
        ...
        rc, writes, _ = wired(tasks, "2")
        out = capsys.readouterr().out
        assert rc == 1
        assert "#1(pending)" in out
        assert writes == [], "a refused start must not mutate the task"
    ...
    def test_force_starts_and_records_the_override(self, wired, capsys):
        ...
        tasks = [_Task("2", blocked_by=["1"]), _Task("1", status="pending")]
        rc, writes, events = wired(tasks, "2", force=True)
        assert rc == 0
        assert writes, "--force must actually start the task"
        assert any(name == "task_start_forced_over_blockers" for name, _ in events)
