...
const mcp = new Server(
  { name: 'telegram', version: '1.1.0-maceff' },
  {
...
// === Permission Relay (MacEff fork addition) ===
// Track pending permission requests: request_id -> chat_id
const pendingPermissions = new Map<string, string>()

// Listen for permission request notifications from Claude Code
log('registering permission relay handler')
mcp.setNotificationHandler(
  PermissionRequestNotificationSchema,
  async (notification) => {
    try {
      log(`PERMISSION REQUEST RECEIVED: ${JSON.stringify(notification.params)}`)
      const params = notification.params as any
      const request_id = params.request_id
      const tool_name = params.tool_name ?? 'unknown'
      const description = params.description ?? ''
      const input_preview = params.input_preview ?? ''
      if (!request_id) return

      // Send to first allowed chat from access.json
      const access = readAccessFile()
      const chatId = access.allowFrom[0]
      if (!chatId) {
        log(`NO ALLOWED CHAT ID — permission relay skipped`)
        return
      }
      log(`RELAY to chatId=${chatId} request_id=${request_id}`)

      pendingPermissions.set(request_id, chatId)

      const msg = `🔐 Permission [${request_id}]\nTool: ${tool_name}\n${description}\n\nSay: approved ${request_id}\nOr: denied ${request_id}`

      bot.api.sendMessage(Number(chatId), msg).then(() => {
        log(`SENT OK`)
      }).catch((err: unknown) => {
        log(`SEND FAILED: ${err}`)
      })
...
bot.on('message:text', async ctx => {
  log(`MESSAGE RECEIVED: from=${ctx.from?.id} text="${ctx.message.text.slice(0,80)}"`)
  ...
  if (allVerdictWords.has(firstWord) && pendingPermissions.size > 0) {
    ...
    if (pendingPermissions.has(requestId)) {
      ...
      return  // Don't pass verdict to normal message handling
    }
  }
...
async function handleInbound(
  ...
  log(`DELIVERING to CC: chat_id=${chat_id} text="${text.slice(0,40)}"`)
  mcp.notification({
    method: 'notifications/claude/channel',
    params: {
      content: text,
    ...
    log(`DELIVERED to CC successfully`)
  }).catch(err => {
